<%@ Page Title="" Language="C#" MasterPageFile="~/Layout/UserMaster.Master" AutoEventWireup="true" CodeBehind="order_History.aspx.cs" Inherits="OnlineHealthCareServices.order_Status" %>
...
<%--                                    <asp:Button ID="Button2" class="btn-block b btn-group-toggle" runat="server" OnClick="EditOrder"  Text="Edit/View" />--%>
                                    <%# Convert.ToDouble(Eval("OrderStatusId")) ==2 || Convert.ToDouble(Eval("OrderStatusId")) ==5 ? "":"<a href='user_Prescription.aspx?PresId="+Eval("PrescriptionDetailId")+"' style='padding: 5px 15px 5px 15px; background-color:#5aba1d; color:#fff; border-radius:5px;'>Edit/View</a>"%>
                                </div>
                                <div class="col-md-6 col-sm-12">
<%--                                    <asp:Button ID="Button3" class="btn btn-success btn-block btn-lg" runat="server" OnClick="CancelOrder" Text="Cancel Order" />--%>
                                         <%# Convert.ToDouble(Eval("OrderStatusId")) !=2 && Convert.ToDouble(Eval("OrderStatusId")) !=4 && Convert.ToDouble(Eval("OrderStatusId")) !=5 ? "<a href='cancel_order.aspx?PresId="+Eval("PrescriptionDetailId")+"' style='padding: 5px 15px 5px 15px; background-color:red; color:#fff; border-radius:5px;'>Cancel Order</a>":""%>
                                         <%# Convert.ToDouble(Eval("OrderStatusId")) ==2 || Convert.ToDouble(Eval("OrderStatusId")) ==5 ? "<a href='user_Prescription.aspx?PresId="+Eval("PrescriptionDetailId")+"' style='padding: 5px 15px 5px 15px; background-color:#5aba1d; color:#fff; border-radius:5px;'>Order Again</a>":""%>
                                </div>
                            </div>
                        </div>
                    </div>
